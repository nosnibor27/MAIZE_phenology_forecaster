...
GDD_8 <- function(max_temp,min_temp){
  if(max_temp>307.15){
    range_1 <- seq(min_temp,max_temp,length.out=8)
    DTT_1 <- ((317.15-range_1)/10)*(307.15-281.15)
    return(mean(DTT_1))
  } else if(max_temp>281.15 & min_temp<281.15){
    range_2 <- seq(min_temp,max_temp,length.out=8)
    DTT_2 <- range_2-281.15
    return(mean(DTT_2))
  } else{
...
GDD_10 <- function(max_temp,min_temp){
  if(max_temp>307.15){
    range_1 <- seq(min_temp,max_temp,length.out=8)
    DTT_1 <- ((317.15-range_1)/10)*(307.15-283.15)
    return(mean(DTT_1))
  } else if(max_temp>281.15 & min_temp<281.15){
    range_2 <- seq(min_temp,max_temp,length.out=8)
    DTT_2 <- range_2-283.15
    return(mean(DTT_2))
  } else{
...
for (n in 1:N){
  ...
  for(j in 1:20){
    #calculating GDD_8 by model
    A[7,j,] <- GDD_8(A[1,j,],A[2,j,])
...
#adding a GDD_10 dimension [variable = 8] to all the historical arrays
for (n in 1:N){
  #copying the array of interest
  A <- get(paste0(locations$town[n],"_hist"))
  #looping through the models
  for(j in 1:20){
    #calculating GDD_10 by model
    A[8,j,] <- GDD_10(A[1,j,],A[2,j,])
    #placing the results back into the array for each town
    assign(paste0(locations$town[n],"_hist"),A)
...
#adding a GDD_10 dimension [variable = 8] to all the RCP 4.5 arrays
for (n in 1:N){
  #copying the array of interest
  A <- get(paste0(locations$town[n],"_45"))
  #looping through the models
...
#adding a GDD_10 dimension [variable = 8] to all the RCP 8.5 arrays
for (n in 1:N){
  #copying the array of interest
  A <- get(paste0(locations$town[n],"_85"))
  #looping through the models
...
for (n in 1:N){
  #copying the array of interest
  A <- get(paste0(locations$town[n],"_45"))
  B <-  get(paste0(locations$town[n],"_RATEIN_45"))
  #creating an indexing array [model, year, stage, planting]
...
for (n in 1:N){
  #copying the array of interest
  A <- get(paste0(locations$town[n],"_85"))
  B <-  get(paste0(locations$town[n],"_RATEIN_85"))
  #creating an indexing array [model, year, stage, planting]
  index_array_fut <- array(dim=c(20,93,7,3))
  for(j in 1:20){
    matrix_1 <- INDEXER(pd_hist_e,A[7,j,],A[8,j,],B)
